...
def tracesToFile(traces, file = "out.afa"):
  """
  converts traces to protein sequence according the method described
  parameters
  ...
  gaussian = norm(traces.mean(1).mean(), traces.std(1).mean())
  thresholds = gaussian.ppf(np.arange(1, len(alphabet))/len(alphabet))
  # thresholds = np.linspace(traces.min, traces.max, len(alphabet)+1)[1:-1]
  length = traces.shape[1]
  letters = np.full(traces.shape, alphabet[0],"S1")
  for (letter, value) in zip(alphabet[1:],thresholds):
    letters[traces > value] = letter
...
def tracesFromFile(traces, file="aligned.afa", cutoff = 0.5):
  ...
  def str2array(string, trace):
    k,j = 0,0
    val = np.zeros(len(string) ,trace.dtype)
    for c in string:
      if c != '-':
        val[j] = trace[k]
        k += 1
